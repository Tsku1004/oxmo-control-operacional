...
function shellHTML() {
  ...
  return `
    ...
    <main class="main">
      <section class="kpis">
        ${kpi("Masa Disponible", masaDisp / 1000, "t", `${disp.length} lotes`, C.green, "ðŸ“¦", 2)}
        ${kpi("Masa Retenida", masaRet / 1000, "t", `${state.lotes.length - disp.length} lotes`, C.red, "ðŸ”’", 2)}
        ${kpi("Fino Mo", finoMoKg / 1000, "t", "Masa x %Mo", C.copper, "â—†", 2)}
        ${kpi("Cu Promedio", cuProm, "%", "Lotes analizados", C.cyan, "âš—ï¸", 2)}
        ${kpi("Total Lotes", state.lotes.length, "", "Todos los sectores", C.blue, "ðŸ—‚ï¸", 0)}
        ${kpi("Sin AnÃ¡lisis", pend.length, "", "Pendientes lab", C.yellow, "ðŸ”¬", 0)}
      </section>
      <nav class="tabs">
        ${[
          ["inventario","ðŸ“¦ Inventario"],["silos","ðŸ­ Silos"],["quimica","âš—ï¸ QuÃ­mica"],["mezclas","âš¡ Mezclas"],
          ["siloHistorial","ðŸ§¾ Historial Silos"],["etiquetas","ðŸ·ï¸ Etiquetas"],["reportes","ðŸ“‹ Reportes"],["alertas","ðŸš¨ Alertas"]
...
function silosHTML() {
  ...
  return `<div style="display:grid;grid-template-columns:minmax(320px,1fr) 360px;gap:14px">
    <div class="grid-cards">${silos.map(s => {
      ...
      const source = s.nivelImportado?.fuente === "infodia" ? `Infodia/ACP ${s.nivelImportado.fecha || ""}` : s.muestras ? "Manual" : "Sin datos";
      return `<div class="card" style="border-top:3px solid ${color}">
...
function bindSilos() {
  ...
  form.addEventListener("submit", e => {
    ...
    state.comunes.push(comun);
    save("oxmo:comunes", state.comunes);
    addHist("ComÃºn de turno ingresado", comun.siloId, `${comun.masa}t ${comun.turno}`, clasificar(comun).color);
    render();
  });
}
